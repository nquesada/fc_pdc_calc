...
############################################################################
## Define the 4x2 different test cases for the Ua, Uc and Va, Vc matrices ##
############################################################################
...
def test2(u1, v1, u2, v2):
    mat_A = np.dot(u1, np.conjugate(np.transpose(v2))) * w_step
    mat_B = np.dot(v1, np.conjugate(np.transpose(u2))) * w_step
...
def test3(u, v):
    ...
    mat_A = mat_A - Uinitial
    mat_B = np.dot(np.conjugate(np.transpose(v)), v) * w_step
...
def test4(u1, v1, u2, v2):
    ...
    mat_B = np.dot(np.conjugate(np.transpose(v2)), u2) * w_step
...
# Check error of the rigorous solution with the reconstructed ua and uc
