...
def construct_u_and_v(r, w_step, wRange):
    """ Constructs the U and V matrices given the R matrix from the analytical
    ...
    u = Uinitial - np.dot(rA, np.dot(np.diag(uDp), \
        np.conjugate(np.transpose(rA))))

    # Construct V
    v = np.dot(np.dot(rA, np.diag(vD)), rBh)
