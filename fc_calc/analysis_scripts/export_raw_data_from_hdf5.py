...
# rc_ana labels the analytical f_c(w,w') solution 
# ra_num labels the numerical f_a(w,w') solution  
# rc_num labels the numerical f_c(w,w') solution 
# va_rig labels the rigorous solution of V_a(w,w') 
# vc_rig labels the rigorous solution of V_c(w,w') 
# ua_rig labels the rigorous solution of U_a(w,w') 
# uc_rig labels the rigorous solution of U_c(w,w') 
...
rc_ana_node = h5file.getNode('/RcfirstItAna', 'zPos'+str(zRange.size-1))
rc_num_node= h5file.getNode('/RcfirstItNum', 'zPos'+str(zRange.size-1))

va_rig_node = h5file.getNode('/Va', 'zPos'+str(zRange.size-1))
ua_rig_node = h5file.getNode('/Ua', 'zPos'+str(zRange.size-1))
vc_rig_node = h5file.getNode('/Vc', 'zPos'+str(zRange.size-1))
uc_rig_node = h5file.getNode('/Uc', 'zPos'+str(zRange.size-1))

ra_ana = ra_ana_node.read()
ra_num = ra_num_node.read()
rc_ana = rc_ana_node.read()
rc_num = rc_num_node.read()

va_rig = va_rig_node.read()
ua_rig = ua_rig_node.read()
vc_rig = vc_rig_node.read()
uc_rig = uc_rig_node.read()
...
## readable format:
f = open(directoryname + '/FC_properties.txt', 'w')
...
fc_properties = {'w_start': w_start, 'w_stop': w_stop, 'w_steps': w_steps, \
...
f = open(directoryname + '/FC_properties.pkl', 'wb')
pickle.dump(fc_properties, f)
f.close()
...
# via this method since transpose itself is also only a view. In this case
# the imaginary values are saved in the rows instead of the columns. This
# is a problem during the import since the import doesn't take this into
# account. One can however solve this by explicitly copying the transposed
# array in a new one which fixes the issue (this is performed in these scripts).
...
np.savetxt(directoryname + "/raw_data/rc_ana.txt.gz", rc_ana.view(float))
np.savetxt(directoryname + "/raw_data/rc_num.txt.gz", rc_num.view(float))
...
np.savetxt(directoryname + "/raw_data/vc_rig.txt.gz", vc_rig.view(float))
np.savetxt(directoryname + "/raw_data/uc_rig.txt.gz", uc_rig.view(float))
